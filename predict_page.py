...
def show_predict_page():
    ...
    if st.button('Predict'):
        try:
            response = requests.post('http://127.0.0.1:5000/predict', data=pickle.dumps(df))
            response.raise_for_status()  # Raise an error for bad status codes
            prediction = pickle.loads(response.content).get('prediction', None)
            ...
            st.error("Failed to decode response from the server.")
